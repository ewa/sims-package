...
REPOSITORY_BASE_URL = "https://luwak.org/x/sims001/"
...
GPG_SIGNING_KEY = "0xAA9BAD6F23BB5492B7CC731868D8E6EF3B1BE979"
...
def upload_public_dir(files, message):
        # Example 0:  rsync it
        cmd = ['/usr/bin/rsync', '-r','-t','-l','--safe-links', '--times', '--compress',
               '--delete-during', '--exclude=\'*~\'', '-h', '--partial', os.getcwd(),
               'eanderso@blackboard.dreamhost.com:/home/eanderso/luwak.org/x/sims001']

        print("About to execute: {cmd}".format(cmd=repr(cmd)))
        rsync_result = subprocess.check_call(cmd)
        print("returned {rv}".format(rv=rsync_result))
...
def get_feeds_rel_path(uri_rel_path):
        sys.stderr.write("processing feeds_rel_path for {p}\n".format(p=uri_rel_path))
...
STRIP_RE = re.compile("^public/")
def get_public_rel_path(feeds_rel_path):
        sys.stderr.write("processing public_rel_path for {p}\n".format(p=feeds_rel_path))
        ## Strip extra "public/"
        newpath = STRIP_RE.sub('',feeds_rel_path)
        #debug
